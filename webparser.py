...
#TODO проходимся по радиусам - для каждого радиуса фильтруем результат парсинга получая всех
# птиц попавших в радиус - берём общий список птиц юзера и фильтруем предыдущий результат,
# оставляя только тех птиц, которых нет у юзера - создаем словарь
# формата { scientific_name: [url1, url2, ...], ...} - рендеринг сообщения пользователю, на каждую
# птицу отдельное сообщение
#TODO команды : /deletelist - удалить список, /setregion - создать район, /getregion - получить
# список районов пользователя с кнопкой удаления каждого
